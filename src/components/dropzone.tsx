      ...
      <div
        {...getRootProps({
          className: cn(
            ...
            'max-w-lg w-full mx-auto',              // limits width and centers
            'min-h-[300px] flex flex-col justify-center', // ⬅️ makes it tall and centers content
            className,
    ...
    <div className={cn('flex flex-col', className)}>
      {files.map((file, idx) => {
        ...
        return (
          <div
            key={`${file.name}-${idx}`}
            className="flex items-center border-b py-2 first:mt-4 last:mb-4 "
          >
            {file.type.startsWith('image/') ? (
              <div className="h-10 w-10 rounded border overflow-hidden shrink-0 bg-muted flex items-center justify-center">
                <Image src={file.preview || ""} alt={file.name} className="object-cover" width={10} height={10} />
              </div>
            ) : (
