...
export default function PropertyForm() {
    ...
    async function checkAuth() {
      try {
        ...
        if (res.data.authenticated) {
          ...
          // ✅ all values destructured first, then passed to setUser
  ...
  // fetching boxes and racks as per selected police station 
  useEffect(() => {
    const fetchRacksAndBoxes = async () => {
      let selectedThana = "";

      if (user.role === "thana admin") {
        selectedThana = user.thana;
      } else {
        selectedThana = formData.policeStation;
      ...
      if (error) {
        console.error("Error fetching racks/boxes:", error.message);
        toast.error("Failed to load racks/boxes");
        return;
      }
    ...
    fetchRacksAndBoxes();
  }, [formData.policeStation, user.role, user.thana]);


  // fetching thanas on initial load and for admin and super admin
  useEffect(() => {
              ...
              <div className={`flex items-center justify-around bg-white w-[80%] max-lg:w-[100%] max-lg:flex-col ${isSubmitted ? "hidden" : "flex"} max-sm:scale-95`}>
                ...
                <div className="min-h-150 max-lg:h-180 w-[75%] flex lg:flex-wrap max-lg:w-full max-lg:flex-col max-md:flex-col max-md:min-h-280 max-md:align-top">
                  ...
                  >
                    ...
                    {/* rack number box number and remarks */}
                    <div className="flex w-full gap-4 max-md:flex-col items-center">
                      <div className="flex flex-col gap-3 w-[48%] max-md:w-[80%] max-sm:w-[90%]">
                        {/* rack number */}
                        <div className="flex items-center">
